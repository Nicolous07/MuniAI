...
export const MessageItem: React.FC<MessageItemProps> = ({ message, onRegenerate, onReadAloud }) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative flex w-full gap-3 px-3 py-4 md:px-6 transition-all ${
    ...
    >
      ...
      <div className="shrink-0 pt-1">
        {isAssistant ? (
          ...
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-cyan-600/30 border border-cyan-400/30 text-cyan-200">
      ...
      <div className={`flex-1 min-w-0 max-w-[88%] md:max-w-[80%] space-y-2 ${isAssistant ? 'items-start' : 'items-end ml-auto'}`}>
        {/* Author Header */}
        <div className={`flex items-center gap-2 ${isAssistant ? 'justify-start' : 'justify-end'}`}>
          ...
          <span className="text-[10px] md:text-xs text-slate-500">{message.timestamp}</span>
        </div>

        {/* Bubble Box */}
        <div
          className={`rounded-2xl p-4 md:p-5 shadow-xl transition-all ${
            isAssistant
              ? 'rounded-tl-xs bg-[#0b101d]/90 border border-indigo-500/20 text-slate-100'
              : 'rounded-tr-xs bg-gradient-to-r from-cyan-900/40 via-indigo-900/50 to-purple-900/40 border border-cyan-500/30 text-slate-100 ml-auto'
          }`}
        >
        ...
        {/* Action Toolbar */}
        <div className={`flex items-center gap-1 pt-1 opacity-80 group-hover:opacity-100 transition-opacity text-slate-400 ${isAssistant ? 'justify-start' : 'justify-end'}`}>
          <button
            onClick={handleCopy}
